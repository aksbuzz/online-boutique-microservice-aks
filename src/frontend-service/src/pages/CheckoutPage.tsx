    ...
    async (_prev: string | null, formData: FormData) => {
      try {
        const res = await checkoutClient.placeOrder({
          ...
          creditCard: {
            creditCardNumber: formData.get('ccNumber') as string,
            creditCardCvv: parseInt(formData.get('ccCvv') as string, 10) || 0,
            creditCardExpirationMonth: parseInt(formData.get('ccExpMonth') as string, 10) || 0,
            creditCardExpirationYear: parseInt(formData.get('ccExpYear') as string, 10) || 0,
          },
        });
        navigate('/success', { state: { order: res.order } });
        return null;
      } catch (e: unknown) {
        // Connect-ES wraps gRPC errors; card declines surface as INVALID_ARGUMENT
  ...
  return (
    <main className="pt-32 pb-24 px-8 md:px-16 max-w-480 mx-auto min-h-screen">
      <div className="max-w-6xl mx-auto">
        ...
        {error && (
          <div className="mb-8 p-4 bg-error-container text-on-error-container rounded font-body text-sm">
            {error}
          </div>
        )}
        ...
        <form action={submitAction} className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-16">
          <div className="space-y-16">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-surface-container-low p-8 rounded-lg">
                <input
                  name="ccNumber"
                  className="md:col-span-2 w-full h-14 px-6 bg-surface-container-lowest border-none focus:outline-none font-body"
                  placeholder="4242 4242 4242 4242"
                  required
                />
                <input
                  name="ccExpMonth"
                  className="w-full h-14 px-6 bg-surface-container-lowest border-none focus:outline-none font-body"
                  placeholder="MM"
                  required
                />
                <input
